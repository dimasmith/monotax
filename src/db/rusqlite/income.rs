...
fn load_all_incomes(conn: &mut Connection) -> anyhow::Result<Vec<Income>> {
    find_incomes(conn, IncomeCriteria::new(&[]))
}

fn find_incomes(conn: &mut Connection, criteria: impl SqlCriteria) -> anyhow::Result<Vec<Income>> {
