...
use crate::domain::tax_payment::{NewTaxPayment, PaymentID, TaxPayment};
...
impl TaxPaymentRepository for SqlxTaxPaymentRepository {
    async fn insert_payment(&mut self, new_payment: NewTaxPayment) -> anyhow::Result<PaymentID> {
...
impl SqlxTaxPaymentRepository {
    async fn generate_id(&mut self) -> anyhow::Result<PaymentID> {
        let pool = &self.pool;
